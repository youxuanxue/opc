...
- [x] 新建 `opc_platform/templates/gzh-curator-accounts.json`，包含 `zhichangluosidao` 与 `yiqichengzhang` 两套 preset
- [x] `weekly-topic-batch.v2.json` 已移除 account 重复默认值（objective/reference_accounts/source_data_dir/target_account.default）
- [x] `templates.py` 已新增 `load_gzh_curator_presets()`，并实现 `gzh_curator_template(..., account_preset)` 强校验、`manifest.account_preset` 写入、`load_template(..., account_preset)` 透传
- [x] `create_opc(..., account_preset)` 已在命令层统一校验；catalog `name` 使用 `manifest.name`；新增 `list_presets(root)`
- [x] `opc_commands.py` 已新增唯一合并函数 `merge_run_inputs(manifest, inputs)`，并在显式输入 `source_data_dir` 时拒绝 `..`
- [x] `run_commands.py` 中 `run_scenario/start_scenario_run/retry_scenario_run/start_retry_scenario_run` 均已在 `execute_run` 前调用 `merge_run_inputs`；`_sync_scenario_with_template` 已从 manifest 读取并透传 `account_preset`
- [x] `executor.py` 已移除 `target_account/reference_accounts` 的硬编码账号回退，仅使用合并后 inputs（缺省为空值）
- [x] `cli.py` 的 `opc create` 已新增 `--account`
- [x] `web_commands.py` 已新增 `GET /api/opc/presets`，且路由顺序在 `/api/opc/{id}` 之前；`POST /api/opc/create` 已透传 `account_preset`
- [x] `web/src/api.ts` 已新增 `presets()`、`createOpc.account_preset`、`config.default_opc_create.account_preset`
- [x] `web/src/App.tsx` 已新增手动创建 OPC 表单（`opc_id/name/template/account_preset`，`gzh-curator` 下必选 preset），并支持默认创建透传 `account_preset`
- [x] 前端空状态已实现：无 OPC 时不请求 planning-defaults，并展示「暂无 OPC，请先创建」
- [x] 已补充测试：preset 创建校验、preset 列表、slug（懿起成长）、run/start 合并行为、路由顺序守护

**当前结论**：本计划列出的实现项已全部落地并通过本地验证，无剩余未完成项。
